...
| 車軸から重心までの高さ | $h$ | 0.080 m |
| 振り子周期（実測） | $T_p$ | 0.75 s |
| 有効慣性モーメント | $I_{\mathrm{eff}}$ | 0.00391 kg·m²（$T_p^2 mgh/4\pi^2$ から逆算） |
...
$I_{\mathrm{eff}}$ は $m$ に比例するので、質量の更新でこの値自体は 0.00279 → 0.00391 kg·m² に変わります。実測しているのは $T_p$ の方であり、モデルに入るのは $a_\theta = 4\pi^2/T_p^2$ という組み合わせなので、$I_{\mathrm{eff}}$ の変化は相殺されます。
...
### MsTimer2 と PWM ピンのタイマ競合

Arduino Uno では **D3 と D11 は Timer2** の PWM 出力です。一方 `MsTimer2` は制御周期の生成に **Timer2 を占有**し、`set()` の中で Timer2 を Normal モードに設定し直します。この状態で D3 / D11 に `analogWrite()` しても、通常の高速 PWM とは異なる挙動になります（`analogWrite(pin, 0)` と `analogWrite(pin, 255)` は `digitalWrite` に落ちるため影響を受けません）。

ピン割り当て上、各モータは**正転と逆転で別のタイマを使う**構成になっています。

| モータ | 正転時に PWM を出すピン | 逆転時に PWM を出すピン |
| --- | --- | --- |
| A | D9（Timer1） | D3（**Timer2**） |
| B | D11（**Timer2**） | D10（Timer1） |

実機は現にバランスしているため、この構成でも実用上は成立しています。ただし左右・正逆でデューティ特性が揃っていない可能性があり、下位ループのゲインを触る前に **オシロスコープで D3 / D9 / D10 / D11 の波形を確認する**ことを推奨します。スケッチのコメントにも「安定版との差分を最小化するため MsTimer2 を意図的に残している」と明記されています。
