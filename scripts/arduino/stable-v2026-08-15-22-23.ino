// ============================================================
// Self-balancing robot: stable-base tuning version
//
// Base:
//   User's corrected 1-wheel + 6-stage input-delay observer code.
//
// Main policy of this version:
//   1) Preserve the fast balance loop that has already become stable.
//   2) Restore the fore/aft LQR gains to the last known good values.
//   3) Keep only the stronger left/right synchronization (kWheelSync = 1.20).
//   4) Keep Serial logging at 20 Hz to reduce timing jitter.
//   5) Provide a SEPARATE slow position-hold outer loop, disabled by default.
//      First verify the baseline with it OFF. Then enable it if desired.
//
// IMPORTANT:
...
// LQR gains
// ------------------------------------------------------------
// Restore the fore/aft gains to the last known-good corrected version.
float gainBodyAngle = -378.545761f;
float gainBodyAngleSpeed = -45.193282f;
float gainWheelAngle = -0.901474f;
float gainWheelAngleSpeed = -5.546318f;

// Delay-state gains are retained for reference / future experiments.
// As in the currently stable corrected version, they are NOT directly
// added to u_WheelAngleSpeed here.
...
void loop() {
  ...
  // Fast balance LQR
  // ==========================================================
  // Position trim only alters the angle reference slowly.
  // When disabled, this is exactly the corrected stable 4-state structure.
  float bodyAngleForControl =
      bodyAngleInRadEstimated
      - bodyAngleTrimFromPosition;

  u_WheelAngleSpeed =
      -gainBodyAngle * bodyAngleForControl
