...
Best real-robot configuration discussed in this project:
    ...
    - motor gain km = 1.99 (measured 2026-08-16, see below)
    - six-stage delay approximation = 0.110 s total
    - LQR Q = diag(10, 1, 1, 1, 0, 0, 0, 0, 0, 0)
    - LQR R = 1.4
...
# Motor gain: how much wheel speed one unit of the LQR command u buys.
#
# Measured on 2026-08-16 with scripts/motor-calibration/km-identification.
# The staircase gave G = 69.9 (wheel A) and 68.0 (wheel B) rad/s per unit
# duty.  The lower loop converts u into a duty with kp + ki*H(s), which is
# 0.0262 (A) and 0.0315 (B) at the unstable-pole frequency 2*pi/T_p, so
#
#     km_A = 69.9 * 0.0262 = 1.83
#     km_B = 68.0 * 0.0315 = 2.14
#
# The model's omega is the average of the two wheels, hence the mean.
#
# Setting this to 1.0 reproduces every design before 2026-08-16, which
# assumed the actuator was exactly as strong as the command implied.  It is
# not: it is about twice as strong, which is why R=1.0 diverged on the real
# robot while R=1.6 and R=1.8 held.
K_MOTOR = 1.99

# NOTE (not yet applied):
# The same measurement puts the motor time constant at 0.44 - 0.82 s, not
# the 0.070 s used here.  That value comes from a free-spinning wheel on a
# stand, and folding it in raises K_theta to about -647, so it is being
# left for a separate step after a on-the-ground re-measurement.
...
print(f"motor gain km      = {K_MOTOR:.6f}")
...
A_c[1, 4] = -a_body_wheel * K_MOTOR

# phi_dot = omega
A_c[2, 3] = 1.0

# Motor:
# omega_dot = -(1/tau)*omega + (km/tau)*z1
A_c[3, 3] = -1.0 / TAU_MOTOR
A_c[3, 4] = +K_MOTOR / TAU_MOTOR
